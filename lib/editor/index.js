...
function Editor (doc) {
  this.document = doc;
  var input = this.input = $('<pre class="input" contenteditable="true">');
  var output = this.output = $('<article class="output">');
...
Editor.prototype.render = function (attrs, callback) {
  var text = attrs.body;
  if (!text) return;
  this.input.get(0).innerText = text;
...
Editor.prototype.onkeyup = throttle(function (e) {
  ...
  var text = this.input.get(0).innerText;

  // HACK: fixes a bug where browsers don't fire a keyup event on empty input so
  // we never get back to completely blank state, instead there's a single line
  // break left over in the input.
  if (text === '\n') {
    text = '';
    this.input.get(0).innerText = '';
  }
